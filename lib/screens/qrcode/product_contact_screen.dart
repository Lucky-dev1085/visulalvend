...
class ProductContactScreen extends StatefulWidget {
  final Function() onSendMessage;

  const ProductContactScreen({Key key, this.onSendMessage, }) : super(key: key);

  @override
  _ProductContactScreenState createState() => _ProductContactScreenState();
}

class _ProductContactScreenState extends State<ProductContactScreen> {
  ...
  onClickedSentMessage() {
    String checkSendMsg = checkSendMessage();
    if(checkSendMsg == ''){
      showToast('Your message sent to server successfully. Thank for your comment.');
      setState(() {
        selectedMachine = spMachineIds.elementAt(0);
        selectedProduct = spProducts.elementAt(0);
        selectedEnquiry = spEnquiryType.elementAt(0);
